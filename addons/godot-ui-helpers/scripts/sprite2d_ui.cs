...
public partial class sprite2d_ui : Node2D
{
	...
	private void createPip(int i, bool startActive = false)
	{
		...
		newPip.Texture = _pipTexture;
		newPip.Scale = _pipScale0;
		newPip.Rotation = _base.Rotation;
		...
		newPip.Position = new Vector2(
			this.Position.X + _pipOffset0.X,
			this.Position.Y + _pipCentreOffset + (_pipSpacing * i) + _pipOffset0.Y) + new Vector2(
	...
	{
		_base = new Sprite2D();
		//_base.Texture = _baseTexture;
		updateBaseTexture();
		scaleBase();
		rotateBase();
		moveBase();
		//_base.Position = new Vector2(
		//	this.Position.X + _baseOffset.X,
		//	this.Position.Y + _baseOffset.Y);
		updateBaseColor();
		AddChild(_base);
	}
	...
	{
		_end = new Sprite2D();
		_end.Texture = _endTexture;
		_end.Scale = _endScale;
		_end.Rotation = Mathf.Lerp(0f, _maxR, Mathf.InverseLerp(0, 360, _endRotationOffset));
		_end.Position = new Vector2(
			this.Position.X + _pipSpacing + _endOffset.X,
			this.Position.Y + _pipSpacing + _endOffset.Y);
		_base.AddChild(_end);
	}
	...
	private void updateBaseColor(int i, int pipsToShow)
	{
		if (_baseColor0 == _baseColor0)
		{
			_base.Modulate = _baseColor0;
		}
		else
		{
			var r = Mathf.Lerp(_pipColor0.R, _pipColor100.R, Mathf.InverseLerp(0, 100, distance));
			var g = Mathf.Lerp(_pipColor0.G, _pipColor100.G, Mathf.InverseLerp(0, 100, distance));
			var b = Mathf.Lerp(_pipColor0.B, _pipColor100.B, Mathf.InverseLerp(0, 100, distance));
			var a = Mathf.Lerp(_pipColor0.A, _pipColor100.A, Mathf.InverseLerp(0, 100, distance));
			_base.Modulate = new Color(r, g, b, a);
		}
	}
	...
	private void createMirrorPip(int i)
	{
		...
		mirrorPip.Position = -_pips[i].Position;
		mirrorPip.Texture = _pips[i].Texture;
		mirrorPip.Modulate = _pips[i].Modulate;    
		mirrorPip.Scale = _pips[i].Scale;
		mirrorPip.Rotation = -_pips[i].Rotation + Mathf.Lerp(0f, _maxR, Mathf.InverseLerp(0, 360, _mirrorRotationOffset));
